...
build/build.img: build/stage1/build.bin build/stage2/build.bin tools/install.py
	...
	echo 'start=2048, type=83' | sfdisk $@
...
debug: build/build.img
	qemu-system-i386 -M pc -m 256M -drive format=raw,file=build/build.img,media=disk -boot c -s -S &
	gdb -ex "target remote localhost:1234" -ex "symbol-file build/stage2/build.elf"
